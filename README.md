...
- Показывает реальные namespace-узлы Проводника в разделах **«Этот компьютер»** и **«Панель навигации (слева)»**.
...
- Показывает или скрывает значки **«Этот компьютер»** и **«Корзина»** на рабочем столе.
- Закрепляет **«Этот компьютер»** и **«Корзину»** в панели навигации Проводника (область переходов слева).
- Добавляет в «Этот компьютер» пункт **«Управление дисками»** — двойной клик открывает системную оснастку `diskmgmt.msc`.
- Применяет изменения кнопкой **«Применить»** (перезапуск Проводника), чтобы они сразу появились в Windows.
...
![Главное окно ShellNsManager: дерево узлов «Этот компьютер» и «Дерево (рабочий стол)»](docs/screenshots/main-window-v1.2.png)
